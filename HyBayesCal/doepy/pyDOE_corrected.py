...
This code was originally published by the following individuals for use with Scilab:
    ...
    website: https://forge.scilab.org/index.php/p/scidoe/source/tree/master/macros
...
def bbdesign_corrected(n, center=None):
    ...
    for i in range(n - 1):
        for j in range(i + 1, n):
            idx += 1
            H[max([0, (idx - 1) * H_fact.shape[0]]):idx * H_fact.shape[0], i] = H_fact[:, 0]
            H[max([0, (idx - 1) * H_fact.shape[0]]):idx * H_fact.shape[0], j] = H_fact[:, 1]
