"""
    Stand alone python script that generates multiple runs for a Telemac simulation based on base Telemac simulation files (*.cas , *.slf , *cli) and the input user parameteres 
    retrieved from a *.xlsx file. Generates a matrix (pd dataframe and .xlsx) with the output values (Velocity or Water depth) from the desired number of simulation runs and simulated calibration parameter and shows figures with graphical variation between the 
